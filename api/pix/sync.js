...
const REPORT_EXPIRES_AFTER_MS = 30 * 60_000;
...
async function processReadyReports(pendingJobs) {
  ...
  const reportsById = new Map(
    availableReports.map((report) => [String(report.id || ""), report])
  );
  let imported = 0;
  let pixFound = 0;
  let processed = 0;
  ...
  for (const job of pendingJobs) {
    ...
    const fileName =
      (report && report.file_name) || job.fileName || job.file_name || "";
    ...
    const requestedAtMs = Number(job.requestedAtMs || 0);
    if (
      requestedAtMs > 0 &&
      now - requestedAtMs > REPORT_EXPIRES_AFTER_MS
    ) {
      await finishReportJob(job.id, {
        status: "expired",
        providerStatus: report && report.status ? report.status : "not_ready"
      });
      continue;
    }
  ...
  return { imported, pixFound, processed, stillPending };
}

async function requestReportIfNeeded(stillPending, reportsProcessed) {
  if (stillPending > 0) return { requested: false, pending: stillPending };
  if (reportsProcessed > 0) return { requested: false, pending: 0 };

  const state = await getReportState();
  const lastRequestedAtMs = Number(state.lastRequestedAtMs || 0);
  if (
    lastRequestedAtMs > 0 &&
    Date.now() - lastRequestedAtMs < REPORT_RETRY_AFTER_MS
  ) {
    return { requested: false, pending: 0 };
  }

  const period = reportPeriod();
  const report = await requestAccountReport(period);
  await saveReportJob(report, period);
  return { requested: true, pending: 1, reportId: String(report.id) };
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const reportConfiguration = await ensureReportConfiguration();
    const pendingJobs = await listPendingReportJobs();
    const reportImport = await processReadyReports(pendingJobs);
    imported += reportImport.imported;

    const reportRequest = await requestReportIfNeeded(
      reportImport.stillPending,
      reportImport.processed
    );
    const reportPending =
      reportRequest.pending > 0 || reportImport.stillPending > 0;
